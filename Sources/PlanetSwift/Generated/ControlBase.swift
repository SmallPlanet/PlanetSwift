//
// Autogenerated by gaxb at 05:06:37 PM on 06/10/20
//

// swiftlint:disable superfluous_disable_command
// swiftlint:disable cyclomatic_complexity
// swiftlint:disable identifier_name
// swiftlint:disable force_cast
// swiftlint:disable type_body_length
// swiftlint:disable function_body_length
// swiftlint:disable line_length
// swiftlint:disable file_length

import UIKit

open class ControlBase: View {

	open func customCopyTo(_ other: Control) {
        // can be overriden by subclasses to provide custom copying
    }

    open override func copy() -> GaxbElement {
        let copied = super.copy() as! Control
        copied.enabled = enabled
        copied.selected = selected
        copied.highlighted = highlighted

		customCopyTo(copied)

        return copied
    }

	open override func visit(_ visitor: (GaxbElement) -> Void) {
        super.visit(visitor)

	}

    override open func setElement(_ element: GaxbElement, key: String) {
        super.setElement(element, key: key)
    }

    override  open func isKindOfClass(_ className: String) -> Bool {
        if className == "Control" {
            return true
        }
        return super.isKindOfClass(className)
    }

	open var enabled: Bool?

    func enabledAsString() -> String {
        return enabled!.toGaxbString()
    }

    open func setEnabled(_ value: String) {
        self.enabled = value == "true"
    }

	open var selected: Bool?

    func selectedAsString() -> String {
        return selected!.toGaxbString()
    }

    open func setSelected(_ value: String) {
        self.selected = value == "true"
    }

	open var highlighted: Bool?

    func highlightedAsString() -> String {
        return highlighted!.toGaxbString()
    }

    open func setHighlighted(_ value: String) {
        self.highlighted = value == "true"
    }

    override open func setAttribute(_ value: String, key: String) {
        super.setAttribute(value, key: key)
        switch key {
        case "enabled":
            setEnabled(value)
        case "selected":
            setSelected(value)
        case "highlighted":
            setHighlighted(value)
        default:
            break
        }
    }

    override open func imprintAttributes(_ receiver: GaxbElement?) -> GaxbElement? {

	if let obj = receiver as? ObjectBase {
            if originalValues["enabled"] != nil && obj.originalValues["enabled"] == nil {
				obj.setAttribute(originalValues["enabled"]!, key: "enabled")
            }
            if originalValues["selected"] != nil && obj.originalValues["selected"] == nil {
				obj.setAttribute(originalValues["selected"]!, key: "selected")
            }
            if originalValues["highlighted"] != nil && obj.originalValues["highlighted"] == nil {
				obj.setAttribute(originalValues["highlighted"]!, key: "highlighted")
            }
       }
       return super.imprintAttributes(receiver)
    }

    override open func attributesXML(_ useOriginalValues: Bool) -> String {
        var xml = ""
        if useOriginalValues {
            for (key, value) in originalValues {
                xml += " \(key)='\(value)'"
            }
        } else {
            if enabled != nil {
                xml += " enabled='\(enabledAsString())'"
            }
                if selected != nil {
                xml += " selected='\(selectedAsString())'"
            }
                if highlighted != nil {
                xml += " highlighted='\(highlightedAsString())'"
            }
            }
        xml += super.attributesXML(useOriginalValues)

        return xml
    }

    override open func sequencesXML(_ useOriginalValues: Bool) -> String {
        var xml = ""
        xml += super.sequencesXML(useOriginalValues)

        return xml
    }

    override open func toXML(_ useOriginalValues: Bool) -> String {
        var xml = "<Control"
        if parent == nil || parent?.xmlns != xmlns {
            xml += " xmlns='\(xmlns)'"
        }

        xml += attributesXML(useOriginalValues)

        let sXML = sequencesXML(useOriginalValues)
        xml += sXML == "" ? "/>": ">\(sXML)</Control>"
        return xml
    }

    override open func toXML() -> String {
        return toXML(false)
    }

    override open func description() -> String {
        return toXML()
    }

}
